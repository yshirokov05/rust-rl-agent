...
def progress_pulse_thread(model_ref, env_ref):
    ...
    while True:
        ...
        try:
            ...
            # Use torch memory stats for GPU Load roughly
            gpu_load = 0.0
            if torch.cuda.is_available():
                gpu_load = (torch.cuda.memory_allocated() / torch.cuda.get_device_properties(0).total_memory) * 100
            ...
            print(f"Status: Training Active. Step: {current_step}. Last Reward: {last_reward:.1f}. GPU Load: {gpu_load:.1f}%", flush=True)
...
def train():
    ...
    run = wandb.init(
        ...
        mode="offline", # Will sync to cloud periodically when wandb sync is run
    ...
    while True: # Loop for OOM Auto-Restart
        try:
            if os.path.exists(latest_model_path):
                ...
                model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="runs/v2_modular", device="auto", batch_size=batch_size)
