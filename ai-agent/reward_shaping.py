...
class RewardShaper:
    ...
    def _calculate_possession_reward(self, observation):
        ...
        tree_dist = float(np.linalg.norm(vector[3:6]))
        ore_dist = float(np.linalg.norm(vector[6:9]))
    ...
    def _calculate_aggression_reward(self, observation, info):
        ...
        if is_attacking and is_moving_forward and tree_dist < 3.0:
            return 0.02 * max(0.0, (3.0 - tree_dist) / 3.0)
