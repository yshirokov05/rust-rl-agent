...
file per bot.  The Carbon plugin advances Tick every server action tick.  A
step is not complete until a telemetry payload with a newer Tick and the same
SessionId arrives.
"""
...
class RustEnv(gym.Env):
    ...
    def __init__(
        ...
        self.alive = True
        self.prev_tree_dist: float | None = None
        self.prev_ore_dist: float | None = None
        self._closed = False

    def _send_actions(self, action: np.ndarray, step_id: int) -> None:
    ...
    def _wait_for_telemetry(self, min_tick: int | None) -> dict[str, Any]:
        deadline = time.monotonic() + self.observation_timeout
        while time.monotonic() < deadline:
            payload = read_json(self.vision_path)
            if payload is not None:
                payload_session = str(payload.get("SessionId", ""))
                if payload_session and payload_session != self.session_id:
        ...
        raise RuntimeError(
            ...
            f"Expected {self.vision_path} with a newer Tick and matching "
            f"SessionId {self.session_id}. Start Carbon and verify "
            "RUST_RL_SHARED_DATA."
        )
    ...
    def _decode_observation(
        ...
        self.has_gathered = bool(data.get("HasGathered", False))
        ...
        info = {
            ...
            "applied_step_id": int(data.get("AppliedStepId", -1)),
            "alive": self.alive,
    ...
    @staticmethod
    def _distance(vector: np.ndarray, start: int, available: bool) -> float:
        if not available:
            return 999.0
        return float(np.linalg.norm(vector[start : start + 3]))
    ...
    def reset(
        ...
        *,
        ...
        self._send_actions(np.zeros(ACTION_SIZE, dtype=np.float32), step_id=0)
        data = self._wait_for_telemetry(min_tick=None)
    ...
    def step(
        ...
        data = self._wait_for_telemetry(min_tick=self.last_tick)
        observation, info = self._decode_observation(data)
        self.last_tick = info["tick"]

        tree_dist = self._distance(
            observation["vector"],
            start=3,
            available=self.current_tree_name != "None",
        )
        ore_dist = self._distance(
            observation["vector"],
            start=6,
            available=self.current_ore_name != "None",
        )

        reward = 0.0
        if self.prev_tree_dist is not None and tree_dist < 900.0:
            reward += float(np.clip(self.prev_tree_dist - tree_dist, -1.0, 1.0))
        if self.prev_ore_dist is not None and ore_dist < 900.0:
            reward += float(np.clip(self.prev_ore_dist - ore_dist, -1.0, 1.0))
        if self.has_gathered:
            # This is based on a confirmed inventory delta from Carbon.
            reward += 10.0
