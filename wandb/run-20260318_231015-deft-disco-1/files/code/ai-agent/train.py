    ...
    while True: # Loop for OOM Auto-Restart
        try:
            ...
            if os.path.exists(latest_model_path):
                ...
                model = PPO.load(latest_model_path, env=env, device="cpu")
                model.device = torch_directml.device()
                # Use native SB3 .to(device) to correctly pipe device logic
                ...
                model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="runs/v2_modular", device="cpu", batch_size=batch_size)
                model.device = torch_directml.device()
                # Use native SB3 .to(device)
                model.to(model.device)
