...
# --- DIRECTML SB3 BUGFIX ---
# PyTorch/SB3 map_location passes strings/devices into plugins expecting raw integers.
_old_device = torch_directml.device
def _patched_device(device_id=None):
    if isinstance(device_id, (str, torch.device)):
        return _old_device(0)
    return _old_device(device_id)
torch_directml.device = _patched_device
# ---------------------------
    ...
    while True: # Loop for OOM Auto-Restart
        try:
            if os.path.exists(latest_model_path):
                ...
                model = PPO.load(latest_model_path, env=env, device="privateuseone:0")
                # Ensure batch size is applied if reduced
                model.batch_size = batch_size 
            else:
                print(f"Starting new training session with batch_size={batch_size}")
                model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="runs/v2_modular", device="privateuseone:0", batch_size=batch_size)
